...
void UBTT_MoveToPatrolPoint::ReceiveExecuteAI(AAIController* OwnerController, APawn* ControlledPawn)
{
    ...
    if (GameUtils::IsValid(Patrol))
    {
        APatrolPathActor* Path = Patrol->GetPatrolPath();
        ...
        if (GameUtils::IsValid(Path))
        {
            ...
            OwnerController->ReceiveMoveCompleted.AddDynamic(this, &UBTT_MoveToPatrolPoint::OnMoveCompleted);
        }
    }
